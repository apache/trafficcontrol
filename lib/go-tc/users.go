...
func (u User) Upgrade() UserV4 {
	var ret UserV4
	ret.AddressLine1 = copyIfNotNil(u.AddressLine1)
	ret.AddressLine2 = copyIfNotNil(u.AddressLine2)
	ret.City = copyIfNotNil(u.City)
	ret.Company = copyIfNotNil(u.Company)
	ret.Country = copyIfNotNil(u.Country)
	ret.Email = copyIfNotNil(u.Email)
	ret.GID = copyIfNotNil(u.GID)
	ret.ID = copyIfNotNil(u.ID)
	ret.LocalPassword = copyIfNotNil(u.LocalPassword)
	ret.PhoneNumber = copyIfNotNil(u.PhoneNumber)
	ret.PostalCode = copyIfNotNil(u.PostalCode)
	ret.PublicSSHKey = copyIfNotNil(u.PublicSSHKey)
	ret.StateOrProvince = copyIfNotNil(u.StateOrProvince)
	ret.Tenant = copyIfNotNil(u.Tenant)
	ret.Token = copyIfNotNil(u.Token)
	ret.UID = copyIfNotNil(u.UID)
	ret.FullName = copyIfNotNil(u.FullName)
...
func (u UserV4) Downgrade() User {
	...
	*ret.Username = u.Username
	...
	ret.City = copyIfNotNil(u.City)
	ret.Company = copyIfNotNil(u.Company)
	ret.ConfirmLocalPassword = copyIfNotNil(u.LocalPassword)
	ret.Country = copyIfNotNil(u.Country)
	ret.Email = copyIfNotNil(u.Email)
	ret.GID = copyIfNotNil(u.GID)
	ret.ID = copyIfNotNil(u.ID)
