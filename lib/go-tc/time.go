...
// TimeRFC3339 wraps standard time.Time to allow indication of invalid times.
// Separate Struct created to Marshal Date format into RFC3339 Format without breaking the existing functions.
type TimeRFC3339 struct {
	time.Time
	Valid bool
}

// Scan implements the database/sql Scanner interface for TimeRFC3339 Struct
func (t *TimeRFC3339) Scan(value interface{}) error {
	t.Time, t.Valid = value.(time.Time)
	return nil
}

// Value implements the database/sql/driver Valuer interface for TimeRFC3339 Struct.
...
// MarshalJSON implements the json.Marshaller interface for TimeRFC3339 Struct.
func (t TimeRFC3339) MarshalJSON() ([]byte, error) {
	return []byte(`"` + t.Time.Format(time.RFC3339) + `"`), nil
}

// UnmarshalJSON implements the json.Unmarshaller interface for TimeRFC3339 Struct
//
// Time structures accept both RFC3339-compliant date/time strings as well as
// the format defined by TimeLayout and Unix(-ish) timestamps. Timestamps are
// expected to be integer numbers that represent milliseconds since Jan 1,
