...
func TestDeliveryServiceRequestNullable_ToConcrete(t *testing.T) {
	assigneeID := 1
	assignee := "assignee"
	var authorID IDNoMod = 2
	author := "author"
	changeType := "change type"
	createdAt := time.Now()
	id := 3
	lastEditedBy := "last edited by"
	var lastEditedByID IDNoMod = 4
	status := RequestStatusDraft
	xmlid := "xmlid"
	expected := DeliveryServiceRequest{
		AssigneeID:      assigneeID,
		Assignee:        assignee,
		AuthorID:        IDNoMod(authorID),
		Author:          author,
		ChangeType:      changeType,
		CreatedAt:       TimeNoModFromTime(createdAt),
		ID:              id,
		LastEditedBy:    lastEditedBy,
		LastEditedByID:  IDNoMod(lastEditedByID),
		LastUpdated:     TimeNoModFromTime(createdAt),
		DeliveryService: DeliveryService{},
		Status:          status,
		XMLID:           xmlid,
	}

	nullable := DeliveryServiceRequestNullable{
		AssigneeID:      &assigneeID,
		Assignee:        &assignee,
		AuthorID:        &authorID,
		Author:          &author,
