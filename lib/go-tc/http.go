...
/*
Represents a "Media Type" as defined by RFC6838, along with some ease-of-use functionality.
...
type MimeType struct {
	// The full name of the MIME Type, e.g. 'application/json'.
	// Usually for printing, it's better to call MimeType.String
	Name string
	// This contains a map of provided parameter names to corresponding values. Note that for
	// MimeTypes constructed with NewMimeType, this will always be initialized, even when empty.
	Parameters map[string]string
}

/*
A convenience function for retrieving and parsing the "quality" parameter of a MimeType.
...
This should only return an error if the MimeType was not initialized properly (i.e. using
NewMimeType); MimeTypes with no "q" parameter implicitly have a "quality" of 1.0.
*/
...
/*
A convenience function for retrieving the "charset" parameter of a MimeType.
...
// Returns only the "main" type of a MimeType.
func (m MimeType) Type() string {
	return strings.SplitN(m.Name, "/", 2)[0]
}

// Returns only the "sub" type of a MimeType.
...
// Returns the MimeType's "facet" if one exists, otherwise an empty string.
...
// Returns the MimeType's "syntax suffix" if one exists, otherwise an empty string.
...
// Implements the Stringer interface using mime.FormatMediaType.
func (m MimeType) String() string {
	return mime.FormatMediaType(m.Name, m.Parameters)
}

/*
Checks whether or not the MimeType is "equal to" some other MimeType, o.
...
/*
Checks whether or not this MimeType is "less than" some other MimeType, o.

This is done using a comparison of "quality value" of the two MimeTypes, as specified in RFC7231.
This WILL panic if *either* this MimeType or the passed o MimeType is "uninitialized" - always
construct MimeTypes with NewMimeType to avoid this.
...
/*
Creates a new MimeType, initializing its Parameters map.
...
/*
Constructs a *sorted* list of MimeTypes from the provided text, which is assumed to be from an
HTTP 'Accept' header. The list is sorted using to SortMimeTypes.
...
/*
Sorts the passed MimeTypes according to their "quality value". See MimeType.Less for more
information on how MimeTypes are compared.
*/
...
// Pre-defined MimeType for JSON data.
var MIME_JSON = MimeType {
	Name: "application/json",
	Parameters: map[string]string{},
}

// Pre-defined MimeType for plain text data.
var MIME_PLAINTEXT = MimeType {
	Name: "text/plain",
	Parameters: map[string]string{"charset": "utf-8"},
}

// Pre-defined MimeType for HTML data.
