...
type DSesSortByName []tc.DeliveryServiceV30
...
func MakeParentDotConfig(
	...
	if !IsTopLevelCache(serverParentCGData) {
		invalidDS := &tc.DeliveryServiceV30{}
...
//      Topology DSes use them for all DSes, MSO and non-MSO.
func getParentDSParams(ds tc.DeliveryServiceV30, profileParentConfigParams map[string]map[string]string) ParentDSParams {
...
func getTopologyGoDirect(ds *tc.DeliveryServiceV30, serverIsLastTier bool) string {
	if !serverIsLastTier {
		return "false"
	}
	if ds.OriginShield != nil && *ds.OriginShield != "" {
...
func GetOriginServersAndProfileCaches(
	...
	dsIDMap := map[int]tc.DeliveryServiceV30{}
	for _, ds := range dses {
		if ds.ID == nil {
	...
	allDSMap := map[int]tc.DeliveryServiceV30{} // all DSes for this server, NOT all dses in TO
...
func GetDSOrigins(dses map[int]tc.DeliveryServiceV30) (map[int]*OriginURI, error) {
...
func makeDSOrigins(dsses []tc.DeliveryServiceServer, dses []tc.DeliveryServiceV30, servers []tc.ServerNullable) map[DeliveryServiceID]map[ServerID]struct{} {
