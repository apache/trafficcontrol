...
type InternalHTTPS string

const (
	InternalHTTPSInvalid InternalHTTPS = ""
	InternalHTTPSNo      InternalHTTPS = "no"
	InternalHTTPSYes     InternalHTTPS = "yes"
	InternalHTTPSNoChild InternalHTTPS = "no-child"
)

const DefaultInternalHTTPS = InternalHTTPSNo

func (ff InternalHTTPS) String() string {
	return string(ff)
}

func StrToInternalHTTPS(str string) InternalHTTPS {
	switch InternalHTTPS(strings.TrimSpace(strings.ToLower(str))) {
	case InternalHTTPSNo:
		return InternalHTTPSNo
	case InternalHTTPSYes:
		return InternalHTTPSYes
	case InternalHTTPSNoChild:
		return InternalHTTPSNoChild
	default:
		return InternalHTTPSInvalid
	}
}
...
//
// Note MetaData may be nil, and often always is for certain files.
type Cfg struct {
	Text        string
	ContentType string
	LineComment string
...
// CfgMetaData is a config file's metadata needed for config generation,
// which is unique and different for each file.
// This will be serialized to JSON, so implementing structs should define JSON field names.
type CfgMetaData interface {
	IsCfgMetaData()
}
...
// dsCacheIsParentChildEdge returns whether the given cache, for the given delivery service:
// - is a parent of other caches
// - is a child of other caches
// - is an 'edge' receiving traffic from external clients.
//
// Note these things are all orthogonal, and most combinations of them are possible.
//
// This considers both Topologies and pre-topology edge/mid hierarchy.
//
func dsCacheIsParentChildEdge(ds *DeliveryService, server *Server, topologies map[TopologyName]tc.Topology, cacheGroups map[tc.CacheGroupName]tc.CacheGroupNullable) (bool, bool, bool, error) {
	// if server.Type == nil {
	// 	return false, errors.New("server has nil type")
	// } else
	if server.Cachegroup == nil {
		return false, false, false, errors.New("server has nil cachegroup")
	}

	if hasTopology := ds.Topology != nil && *ds.Topology != ""; !hasTopology {
		// TODO handle edges parented to edges? Do we have the data necessary?
		isParent := tc.CacheTypeFromString(server.Type) != tc.CacheTypeEdge
		isChild := tc.CacheTypeFromString(server.Type) == tc.CacheTypeEdge
		isEdge := tc.CacheTypeFromString(server.Type) == tc.CacheTypeEdge
		return isParent, isChild, isEdge, nil
	}

	topology, ok := topologies[TopologyName(*ds.Topology)]
	if !ok {
		return false, false, false, errors.New("ds topology '" + *ds.Topology + "' not in topologies")
	}

	topoIncludesServer, err := topologyIncludesServerNullable(topology, server)
	if err != nil {
		return false, false, false, errors.New("checking if ds topology '" + *ds.Topology + "' includes server: " + err.Error())
	}
	if !topoIncludesServer {
		// if the server isn't in the topology, return that it isn't a parent with no error.
		// TODO warn? otherwise provide info as to what happened? This may be normal
		return false, false, false, nil
	}

	topoPlacement, err := getTopologyPlacement(tc.CacheGroupName(*server.Cachegroup), topology, cacheGroups, ds)
	if err != nil {
		return false, false, false, errors.New("ds topology '" + *ds.Topology + "' error getting placement: " + err.Error())
	}
	...
	isParent := !topoPlacement.IsFirstCacheTier
	isChild := !topoPlacement.IsLastCacheTier
	isEdge := topoPlacement.IsFirstCacheTier
