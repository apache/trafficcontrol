...
func addKbps(statHistory map[enum.CacheName][]cache.Result, dsStats Stats, lastKbpsStats StatsLastKbps, dsStatsTime time.Time) (Stats, StatsLastKbps, error) {
	for dsName, stat := range dsStats.DeliveryService {
		...
		for cgName, cacheStats := range stat.CacheGroups {
			...
			if lastKbpsStatExists && lastKbpsData.Bytes != 0 {
				cacheStats.Kbps.Value = float64(cacheStats.OutBytes.Value-lastKbpsData.Bytes) / BytesPerKbps / dsStatsTime.Sub(lastKbpsData.Time).Seconds()
			}
		...
		if lastKbpsStatExists && lastKbpsStat.Total.Bytes != 0 && totalChanged {
			stat.TotalStats.Kbps.Value = float64(stat.TotalStats.OutBytes.Value-lastKbpsStat.Total.Bytes) / BytesPerKbps / dsStatsTime.Sub(lastKbpsStat.Total.Time).Seconds()
	...
	for cacheName, results := range statHistory {
		...
		kbps := float64(outBytes-lastCacheKbpsData.Bytes) / BytesPerKbps / result.Time.Sub(lastCacheKbpsData.Time).Seconds()
