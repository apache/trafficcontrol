...
type LastKbpsData struct {
	Kbps  float64 // TODO determine if necessary, remove if not
	Bytes int64
	Time  time.Time
}
...
func addKbps(dsStats DsStats, lastKbpsStats DsStatsLastKbps, dsStatsTime time.Time) (DsStats, DsStatsLastKbps, error) {
	for dsName, iStat := range dsStats.DeliveryService {
		...
		for cgName, cacheStats := range stat.CacheGroups {
			...
			stat.CacheGroups[cgName] = cacheStats // TODO determine if necessary
		}
		...
		for cacheType, cacheStats := range stat.Type {
			...
			stat.Type[cacheType] = cacheStats // TODO determine if necessary
		}
		...
		lastKbpsStats[dsName] = lastKbpsStat   // TODO determine if necessary
		dsStats.DeliveryService[dsName] = stat // TODO determine if necessary
	}
