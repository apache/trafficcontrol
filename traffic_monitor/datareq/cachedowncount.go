...
func cacheDownCount(caches map[tce.CacheName]tc.IsAvailable, toServers map[string]tc.TrafficServer) int {
	count := 0
	for cache, available := range caches {
		if !available.IsAvailable && tce.CacheStatusFromString(toServers[string(cache)].ServerStatus) == tce.CacheStatusReported {
			count++
		}
	}
	return count
