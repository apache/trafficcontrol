...
type DurationMap struct {
	durationMap *map[tce.CacheName]time.Duration
	m           *sync.RWMutex
}

// Copy copies this duration map.
func CopyDurationMap(a map[tce.CacheName]time.Duration) map[tce.CacheName]time.Duration {
	b := map[tce.CacheName]time.Duration{}
	for k, v := range a {
		b[k] = v
	}
	return b
...
func NewDurationMap() DurationMap {
	m := map[tce.CacheName]time.Duration{}
...
func (o *DurationMap) Get() map[tce.CacheName]time.Duration {
	o.m.RLock()
	defer o.m.RUnlock()
...
func (o *DurationMap) Set(d map[tce.CacheName]time.Duration) {
	o.m.Lock()
	*o.durationMap = d
	o.m.Unlock()
}
