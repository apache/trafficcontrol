...
sub register {
	...
	$app->renderer->add_helper(
	  ...
	  # this is the best query for the job, even though you can't search in it
			my $rs = $self->db->resultset('RascalHostsByCdn')
				->search( { 'cdn_name' => 'ALL' } );
			...
			if ( exists $args->{hostname} ) {
				...
				$port
					= exists( $args->{port} )
					? $args->{port}
					: 80;    # port is optional deaults to 80
			}

			$traffic_monitor_connection
				= new Utils::Rascal( $hostname, $port );
			my $proxy_param = $self->db->resultset('Parameter')->search(
				{   -and => [
