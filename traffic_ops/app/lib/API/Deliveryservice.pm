	...
	while ( my $row = $rs_data->next ) {
		...
		push(
			@data, {
				...
				"signed"               => defined( $row->signing_algorithm ) ? ( $row->signing_algorithm eq "url_sig" ? \1 : \0 ) : undef,
	...
	if ($rs) {
		...
		push(
			@response, {
				...
				"signed"                   => defined( $rs->signing_algorithm ) ? ( $rs->signing_algorithm eq "url_sig" ) : undef,
	...
	if ($insert) {
		...
		push(
			@response, {
				...
				"signed"                   => defined( $insert->signing_algorithm ) ? ( $insert->signing_algorithm eq "url_sig" ) : undef,
