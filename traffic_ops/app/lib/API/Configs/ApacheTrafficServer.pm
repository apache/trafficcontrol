...
sub parent_ds_data {
	...
	if ( $server_obj->type->name =~ m/^MID/ ) {
		...
		while ( my $dsinfo = $rs_dsinfo->next ) {
			...
			if ( defined( $dsinfo->profile ) ) {
				my $dsparamrs =
					$self->db->resultset('ProfileParameter')->search( { profile => $dsinfo->profile }, { prefetch => [ 'profile', 'parameter' ] } );
				while ( my $prow = $dsparamrs->next ) {
					$response_obj->{dslist}->[$j]->{'param'}->{ $prow->parameter->config_file }->{ $prow->parameter->name } = $prow->parameter->value;
				}
			}
		...
		while ( my $dsinfo = $rs_dsinfo->next ) {
			...
			if ( defined( $dsinfo->profile ) ) {
				my $dsqstring =
					$self->db->resultset('ProfileParameter')
					->search(
					{ -and => [ 'parameter.config_file' => 'parent.config', 'parameter.name' => 'psel.qstring_handling', profile => $dsinfo->profile ] },
