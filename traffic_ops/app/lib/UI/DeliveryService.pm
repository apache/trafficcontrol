...
sub get_cdn_domain {
	...
	my $cdn_domain = $self->db->resultset('Parameter')->search(
		...
		{
			join => {
				profile_parameters => { profile => { deliveryservices => undef } }
			},
			distinct => 1
		}
...
sub check_deliveryservice_input {
	...
	if ( $typename =~ /^DNS/ ) {
		...
		if ( defined( $self->param('ds.tr_request_headers') ) && $self->param('ds.tr_request_headers') ne "" ) {
...
sub update {
	...
	if ( $self->check_deliveryservice_input() ) {
		...
		my %hash = (
			...
			global_max_mbps        => $self->hr_string_to_mbps( $self->paramDefault( 'ds.global_max_mbps', 0 ) ),
			global_max_tps         => $self->paramDefault( 'ds.global_max_tps', 0 ),
			...
			tr_response_headers    => $self->paramDefault( 'ds.tr_response_headers', undef ),
			tr_request_headers     => $self->paramDefault( 'ds.tr_request_headers', undef ),
		...
		&stash_role($self);
		my $rs_ds =
			$self->db->resultset('Deliveryservice')->search( { 'me.id' => $id }, { prefetch => [ { 'type' => undef }, { 'profile' => undef } ] } );
		my $data         = $rs_ds->single;
		my $cdn_domain   = &get_cdn_domain( $self, $id );
		my $server_count = $self->db->resultset('DeliveryserviceServer')->search( { deliveryservice => $id } )->count();
		my $static_count = $self->db->resultset('Staticdnsentry')->search( { deliveryservice => $id } )->count();
		...
		my $insert = $self->db->resultset('Deliveryservice')->create(
			{
				xml_id                 => $self->param('ds.xml_id'),
				display_name           => $self->param('ds.display_name'),
				dscp                   => $self->paramDefault( 'ds.dscp', 0 ),
				signed                 => $self->param('ds.signed'),
				qstring_ignore         => $self->param('ds.qstring_ignore'),
				geo_limit              => $self->param('ds.geo_limit'),
