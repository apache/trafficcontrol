...
sub run_ut {
	...
	my $tenant_id = undef;#TO BE SET after the merge with the user tenancy PR
	...
	ok $t->post_ok('/api/1.2/cdns' => {Accept => 'application/json'} => json => {
        	"name" => "cdn_test", "dnssecEnabled" => \1, "domainName" => "testcdn.kabletown.net", "tenantId" => $tenant_id })
	    ->status_is(200)->or( sub { diag $t->tx->res->content->asset->{content}; } )
	...
	ok $t->put_ok('/api/1.2/cdns/' . $cdn_id  => {Accept => 'application/json'} => json => {
        	"name" => "cdn_test2", "dnssecEnabled" => \1, "domainName" => "testcdn2.kabletown.net", "tenantId" => $tenant_id
        	})
	    ->status_is(200)->or( sub { diag $t->tx->res->content->asset->{content}; } )
	...
	ok $t->put_ok('/api/1.2/cdns/' . $cdn_id  => {Accept => 'application/json'} => json => {
        	"name" => "cdn_test3", "domainName" => "testcdn2.kabletown.net"
        	})
	    ->status_is(404)->or( sub { diag $t->tx->res->content->asset->{content}; } );

	#behvior when no tenant id set
	ok $t->post_ok('/api/1.2/cdns' => {Accept => 'application/json'} => json => {
        	"name" => "cdn_test", "domainName" => "testcdn.kabletown.net", "dnssecEnabled" => \1})
	    ->status_is(200)->or( sub { diag $t->tx->res->content->asset->{content}; } )
	...
	ok $t->put_ok('/api/1.2/cdns/' . $cdn_id  => {Accept => 'application/json'} => json => {
        ...
        #removing tenancy explictly
       	ok $t->put_ok('/api/1.2/cdns/' . $cdn_id  => {Accept => 'application/json'} => json => {
        	"name" => "cdn_test2", "dnssecEnabled" => \1, "domainName" => "testcdn2.kabletown.net", "tenantId" => undef
        	})
	    ->status_is(200)->or( sub { diag $t->tx->res->content->asset->{content}; } )
	    ->json_is( "/response/name" => "cdn_test2" )
	    ->json_is( "/response/tenantId", undef)->or( sub { diag $t->tx->res->content->asset->{content}; } )
