...
ok $t->post_ok( '/api/1.2/user/login', json => { u => Test::TestHelper::ADMIN_USER, p => Test::TestHelper::ADMIN_USER_PASSWORD } )->status_is(200),
	"Can the admin login?";

ok $t->get_ok( '/api/1.2/user/2/deliveryservices/available.json',
...
ok $t->get_ok('/api/1.2/user/current.json')->status_is(200)->or( sub { diag $t->tx->res->content->asset->{content}; } )
	->json_is( "/response/username", Test::TestHelper::ADMIN_USER ), "Verify the admin can reach the current user";

ok $t->post_ok( '/api/1.2/user/current/update', json => { user => { addressLine1 => 'newaddress', email => 'testportal@kabletown.com', role => 4 } } )
...
ok $t->post_ok( '/api/1.2/user/current/update', json => { user => { addressLine1 => 'newaddress', email => 'testportal1@kabletown.com', role => 4 } } )
	->status_is(200)->or( sub { diag $t->tx->res->content->asset->{content}; } )->json_is( "/alerts/0/level", "success" ),
...
$t->post_ok( '/api/1.2/user/current/update', json => { address_line1 => 'newaddress' } )->status_is(400)
...
ok $t->post_ok( '/api/1.2/user/current/update', json => { user => { email => '@kabletown.com' } } )->status_is(400)
	->or( sub { diag $t->tx->res->content->asset->{content}; } )->json_is( "/alerts/0/level", "error" ),
