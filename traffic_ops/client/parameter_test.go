...
func TestParameter(t *testing.T) {
	...
	for _, parameter := range parameterList.Response {
		...
		if len(parameter.ConfigFile) == 0 {
			t.Error("ConfigFile parameter is null for parameter: " + name)
		}
		if len(parameter.LastUpdated) == 0 {
			t.Error("LastUpdate parameter is null for parameter: " + name)
		}
		if len(parameter.Value) == 0 {
			t.Error("Value parameter is null for parameter: " + name)
		}
	}
}
