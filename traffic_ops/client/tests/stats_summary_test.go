...
func TestStatsSummary(t *testing.T) {
	...
	Context(t, "Given the need to test a successful Traffic Ops request for Stats Summary")
	...
	if len(stats) != 1 {
		Error(t, "Should get back \"1\" Parameter, got: %d", len(stats))
	} else {
	...
	for _, s := range stats {
		if s.StatName != "test-stat" {
			Error(t, "Should get back \"test-stat\" for \"StatName\", got: %s", s.StatName)
		} else {
			Success(t, "Should get back \"test-stat\" for \"StatName\"")
		}

		if s.DeliveryService != "test-ds1" {
			Error(t, "Should get back \"test-ds1\" for \"DeliveryService\", got: %s", s.DeliveryService)
		} else {
			Success(t, "Should get back \"test-ds1\" for \"DeliveryService\"")
		}

		if s.StatValue != "3.1415" {
			Error(t, "Should get back \"3.1415\" for \"StatValue\", got: %s", s.StatValue)
		} else {
			Success(t, "Should get back \"3.1415\" for \"StatValue\"")
		}

		if s.CDNName != "test-cdn" {
			Error(t, "Should get back \"test-cdn\" for \"CDNName\", got: %s", s.CDNName)
		} else {
			Success(t, "Should get back \"test-cdn\" for \"CDNName\"")
		}
	}
}
...
func TestStatsSummaryUnauthorized(t *testing.T) {
	...
	Context(t, "Given the need to test a failed Traffic Ops request for Stats Summary")
