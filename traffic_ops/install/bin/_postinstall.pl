...
sub setupMaxMind {
    ...
    if ( $setupMaxmind =~ /^y(?:es)?/ ) {
        ...
        $result = InstallUtils::execCommand("/usr/bin/curl https://geolite.maxmind.com/download/geoip/database/GeoLite2-City.mmdb.gz");
        if ( $result != 0 ) {
            InstallUtils::logger( "Failed to download MaxMind data", "error" );
        }

        $result = InstallUtils::execCommand("/usr/bin/curl https://geolite.maxmind.com/download/geoip/database/GeoLiteCityv6-beta/GeoLiteCityv6.dat.gz");
        if ( $result != 0 ) {
            InstallUtils::logger( "Failed to download MaxMind data", "error" );
        }
    }
