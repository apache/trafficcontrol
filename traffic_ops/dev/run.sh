...
cd "$TC"
make
cd "$TC/traffic_ops/dev"

"$ADMIN" -c ./dbconf.yml -s ../app/db/create_tables.sql -S ../app/db/seeds.sql -p ../app/db/patches.sql -m ../app/db/migrations reset
"$ADMIN" -c ./dbconf.yml -s ../app/db/create_tables.sql -S ../app/db/seeds.sql -p ../app/db/patches.sql -m ../app/db/migrations upgrade
"$ADMIN" -v -c ./traffic.vault.dbconf.yml -s ./../app/db/trafficvault/create_tables.sql -m ../app/db/trafficvault/migrations reset
"$ADMIN" -v -c ./traffic.vault.dbconf.yml -s ./../app/db/trafficvault/create_tables.sql -m ../app/db/trafficvault/migrations upgrade
...
dlv --accept-multiclient --continue --listen=:6444 --headless --api-version=2 debug -- --cfg=../dev/cdn.json --dbcfg=../dev/db.config.json &
