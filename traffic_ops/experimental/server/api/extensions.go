...
func getExtensionsById(id string, db *sqlx.DB) (interface{}, error) {
	ret := []Extensions{}
	arg := Extensions{}
	arg.Name = id
...
func putExtensions(id string, payload []byte, db *sqlx.DB) (interface{}, error) {
	var v Extensions
	err := json.Unmarshal(payload, &v)
...
func delExtensions(id string, db *sqlx.DB) (interface{}, error) {
	arg := Extensions{}
	arg.Name = id
	result, err := db.NamedExec("DELETE FROM extensions WHERE name=:id", arg)
	if err != nil {
		log.Println(err)
		return nil, err
	}
