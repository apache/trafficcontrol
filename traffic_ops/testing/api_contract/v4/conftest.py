...
JSONData: TypeAlias = Union[dict[str, object],
                            list[object], bool, int, float, str | None]
...
class APIVersion(NamedTuple):
    ...
    def from_string(ver_str: str) -> "APIVersion":
        ...
        if len(parts) != 2:
            raise ValueError(
                "invalid version; must be of the form '{{major}}.{{minor}}'")
...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    parser.addoption(
        ...
        default=os.path.join(os.path.dirname(
            __file__), "request_template.json")
    )
    parser.addoption(
        "--response-template",
...
def coalesce_config(
    ...
    if file_contents:
        ...
        if file_value is not None:
            raise ValueError(
                f"incorrect value; want: 'str', got: '{type(file_value)}'")
...
def parse_to_url(raw: str) -> tuple[APIVersion, int]:
    ...
    if parsed.path and parsed.path != "/":
        ...
        if not ver_str:
            raise ValueError(
                f"invalid API path: {parsed.path} (should be e.g. '/api/4.0')")
...
def to_data(pytestconfig: pytest.Config) -> ArgsType:
    ...
    if isinstance(cfg_path, str):
        try:
            ...
            raise ValueError(
                f"could not read configuration file at '{cfg_path}'") from read_err
    ...
    to_user = coalesce_config(pytestconfig.getoption(
        "--to-user"), "user", session_data, "TO_USER")
    if not to_user:
        raise ValueError(
    ...
    to_url = coalesce_config(pytestconfig.getoption(
        "--to-url"), "url", session_data, "TO_USER")
    if not to_url:
        raise ValueError(
...
def request_prerequiste_data(pytestconfig: pytest.Config, request: pytest.FixtureRequest
    ...
    if not isinstance(data, dict):
        raise TypeError(
            f"request template data must be an object, not '{type(data)}'")
    request_template = data[request.param]
    if not isinstance(request_template, list):
        raise TypeError(
            f"Request template data must be a list, not '{type(request_template)}'")
...
def response_template_data(pytestconfig: pytest.Config
    ...
    if not isinstance(response_template, dict):
        raise TypeError(
            f"Response template data must be an object, not '{type(response_template)}'")
...
def cdn_post_data(to_session: TOSession, request_template_data: list[JSONData]
    ...
    try:
        ...
        raise TypeError(
            "malformed prerequisite data; no CDNs present in 'cdns' array property") from e

    if not isinstance(cdn, dict):
        raise TypeError(
            f"malformed prerequisite data; CDNs must be objects, not '{type(cdn)}'")
    ...
    try:
        ...
        if not isinstance(domain_name, str):
            raise TypeError(
                f"domainName must be str, not '{type(domain_name)}")
    ...
    response: tuple[JSONData,
                    requests.Response] = to_session.create_cdn(data=cdn)
    try:
        resp_obj = response[0]
...
def cachegroup_post_data(to_session: TOSession, request_template_data: list[JSONData]
    ...
    try:
        ...
        if not isinstance(type_data, list):
            raise TypeError(
                "malformed API response; 'response' property not an array")
        first_type = type_data[0]
        if not isinstance(first_type, dict):
            raise TypeError(
                "malformed API response; first Type in response is not an object")
    ...
    response: tuple[JSONData, requests.Response] = to_session.create_cachegroups(
        data=cachegroup)
    try:
        resp_obj = response[0]
        if not isinstance(resp_obj, dict):
            raise TypeError(
                "malformed API response; cache group is not an object")
        return resp_obj
    except IndexError:
...
def parameter_post_data(to_session: TOSession, request_template_data: list[JSONData]
    ...
    logger.info("New parameter data to hit POST method %s",
                request_template_data)
    # Hitting cdns POST methed
    response: tuple[JSONData, requests.Response] = to_session.create_parameter(
        data=parameter)
    try:
        resp_obj = response[0]
        if not isinstance(resp_obj, dict):
            raise TypeError(
                "malformed API response; parameter is not an object")
        return resp_obj
    except IndexError:
        logger.error(
            "No Parameter response data from parameters POST request.")
        sys.exit(1)
...
def role_post_data(to_session: TOSession, request_template_data: list[JSONData]
    ...
    try:
        ...
        if not isinstance(description, str):
            raise TypeError(
                f"description must be str, not '{type(description)}")
    ...
    response: tuple[JSONData,
                    requests.Response] = to_session.create_role(data=role)
...
def profile_post_data(to_session: TOSession, request_template_data: list[JSONData]
    ...
    if not isinstance(profile, dict):
        raise TypeError(
            f"malformed prerequisite data; profile must be objects, not '{type(profile)}'")
    ...
    try:
        ...
        if not isinstance(first_cdn, dict):
            raise TypeError(
                "malformed API response; first CDN in response is not an object")
    ...
    response: tuple[JSONData, requests.Response] = to_session.create_profile(
        data=profile)
    try:
        resp_obj = response[0]
        if not isinstance(resp_obj, dict):
...
def division_post_data(to_session: TOSession, request_template_data: list[JSONData]
    ...
    response: tuple[JSONData, requests.Response] = to_session.create_division(
        data=division)
    try:
        resp_obj = response[0]
        if not isinstance(resp_obj, dict):
            raise TypeError(
                "malformed API response; division is not an object")
        return resp_obj
    except IndexError:
...
def region_post_data(to_session: TOSession, request_template_data: list[JSONData]
	...
	try:
		...
		logger.error("No Region response data from division POST request.")
		sys.exit(1)
...
def phys_locations__post_data(to_session: TOSession, request_template_data: list[JSONData]
    ...
    response: tuple[JSONData, requests.Response] = to_session.create_region(
        data=phys_location)
    try:
        resp_obj = response[0]
        if not isinstance(resp_obj, dict):
            raise TypeError(
                "malformed API response; phys_location is not an object")
        return resp_obj
    except IndexError:
        logger.error(
            "No phys_location response data from phys_location POST request.")
        sys.exit(1)
