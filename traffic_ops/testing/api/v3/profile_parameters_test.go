		...
		methodTests := utils.V3TestCaseT[[]tc.ProfileParameter]{
			"GET": {
				...
				"OK when VALID request": {
					...
					RequestParams: url.Values{
						...
						"parameterId": {strconv.Itoa(GetParameterID(t, "peers.polling.interval", "tm-config.txt", "60")())}},
			...
			"POST": {
				...
				"BAD REQUEST when MISSING PROFILEID field": {
					ClientSession: TOSession,
					RequestBody: []tc.ProfileParameter{{
						ParameterID: GetParameterID(t, "health.threshold.queryTime", "tm.properties", "1000")(),
					}},
					Expectations: utils.CkRequest(utils.HasError(), utils.HasStatus(http.StatusBadRequest)),
				},
				...
				"BAD REQUEST when ALREADY EXISTS": {
					...
					RequestBody: []tc.ProfileParameter{{
						...
						ParameterID: GetParameterID(t, "health.threshold.availableBandwidthInKbps", "tm.properties", ">1750000")(),
					}},
					Expectations: utils.CkRequest(utils.HasError(), utils.HasStatus(http.StatusBadRequest)),
				},
			},
