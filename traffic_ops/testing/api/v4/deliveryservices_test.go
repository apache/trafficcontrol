...
func cleanUp(t *testing.T, ds tc.DeliveryServiceV40, oldCDNID int, newCDNID int) {
...
func SSLDeliveryServiceCDNUpdateTest(t *testing.T) {
	...
	customDS := tc.DeliveryServiceV40{}
	...
	ds, _, err := TOSession.CreateDeliveryServiceV40(customDS)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, _, err = TOSession.UpdateDeliveryServiceV40WithHdr(*ds.ID, ds, nil)
	if err == nil {
...
func GetTestDeliveryServicesIMSAfterChange(t *testing.T, header http.Header) {
	_, reqInf, err := TOSession.GetDeliveryServicesV40WithHdr(header, nil)
	if err != nil {
...
func PostDeliveryServiceTest(t *testing.T) {
	...
	_, _, err := TOSession.CreateDeliveryServiceV40(ds)
	if err == nil {
	...
	_, reqInf, err := TOSession.CreateDeliveryServiceV40(ds)
	if err == nil {
...
func GetTestDeliveryServices(t *testing.T) {
	actualDSes, _, err := TOSession.GetDeliveryServicesV40WithHdr(nil, nil)
	if err != nil {
		t.Errorf("cannot GET DeliveryServices: %v - %v", err, actualDSes)
	}
	actualDSMap := make(map[string]tc.DeliveryServiceV40, len(actualDSes))
...
func GetInactiveTestDeliveryServices(t *testing.T) {
	...
	inactiveDSes, _, err := TOSession.GetDeliveryServicesV40WithHdr(nil, params)
	if err != nil {
	...
	activeDSes, _, err := TOSession.GetDeliveryServicesV40WithHdr(nil, params)
	if err != nil {
...
func GetTestDeliveryServicesCapacity(t *testing.T) {
	...
	actualDSMap := map[string]tc.DeliveryServiceV40{}
	for _, ds := range actualDSes {
		actualDSMap[*ds.XMLID] = ds
...
func UpdateTestDeliveryServices(t *testing.T) {
	...
	dses, _, err := TOSession.GetDeliveryServicesV40WithHdr(nil, nil)
	if err != nil {
		t.Errorf("cannot GET Delivery Services: %v", err)
	}

	remoteDS := tc.DeliveryServiceV40{}
	found := false
	for _, ds := range dses {
	...
	if updateResp, _, err := TOSession.UpdateDeliveryServiceV40WithHdr(*remoteDS.ID, remoteDS, nil); err != nil {
	...
	apiResp, _, err := TOSession.GetDeliveryServicesV40WithHdr(nil, params)
	if err != nil {
...
func UpdateNullableTestDeliveryServices(t *testing.T) {
	...
	var remoteDS tc.DeliveryServiceV40
	found := false
	for _, ds := range dses {
...
func UpdateDeliveryServiceWithInvalidTopology(t *testing.T) {
	...
	found := false
	var nonCSDS *tc.DeliveryServiceV40
	for _, ds := range dses {
		...
		if *ds.Type == tc.DSTypeClientSteering {
			...
			if _, _, err := TOSession.UpdateDeliveryServiceV40WithHdr(*ds.ID, ds, nil); err == nil {
				t.Errorf("assigning topology to CLIENT_STEERING delivery service - expected: error, actual: no error")
			}
		} else if nonCSDS == nil {
			nonCSDS = new(tc.DeliveryServiceV40)
			*nonCSDS = ds
		}
	}
	if !found {
	...
	_, inf, err := TOSession.UpdateDeliveryServiceV40WithHdr(*nonCSDS.ID, *nonCSDS, nil)
	if err == nil {
	...
	dses, _, err = TOSession.GetDeliveryServicesV40WithHdr(nil, params)
	if err != nil {
	...
	ds.Topology = &top
	_, reqInf, err := TOSession.UpdateDeliveryServiceV40WithHdr(*ds.ID, ds, nil)
	if err == nil {
	...
	dses, _, err = TOSession.GetDeliveryServicesV40WithHdr(nil, url.Values{"xmlId": {xmlId}})
	if err != nil {
	...
	ds.Topology = nil
	ds, _, err = TOSession.UpdateDeliveryServiceV40WithHdr(*ds.ID, ds, nil)
	if err != nil {
	...
	*server.ProfileID = profileCopy.ExistingID
	...
	ds, reqInf, err = TOSession.UpdateDeliveryServiceV40WithHdr(*ds.ID, ds, nil)
	if err != nil {
		t.Fatalf("updating Delivery Service %s: %s", xmlId, err.Error())
	}
}
...
func UpdateDeliveryServiceWithInvalidRemapText(t *testing.T) {
	...
	if _, _, err := TOSession.UpdateDeliveryServiceV40WithHdr(*remoteDS.ID, remoteDS, nil); err == nil {
...
func UpdateValidateORGServerCacheGroup(t *testing.T) {
	...
	//Get the correct DS
	remoteDS, _, err := TOSession.GetDeliveryServicesV40WithHdr(nil, params)
	if err != nil {
		t.Errorf("cannot GET Delivery Services: %v", err)
	}
	...
	ds, reqInf, err := TOSession.UpdateDeliveryServiceV40WithHdr(*remoteDS[0].ID, remoteDS[0], nil)
	if err == nil {
	...
	_, _, err = TOSession.UpdateDeliveryServiceV40WithHdr(*remoteDS[0].ID, remoteDS[0], nil)
	if err != nil {
...
func getByTenants(tenantID int, expectedCount int) error {
	...
	deliveryServices, _, err := TOSession.GetDeliveryServicesV40WithHdr(nil, params)
	if err != nil {
		return err
	}
...
func DeleteTestDeliveryServices(t *testing.T) {
	...
	for _, testDS := range testData.DeliveryServices {
		...
		foundDS, _, err := TOSession.GetDeliveryServicesV40WithHdr(nil, params)
		if err != nil {
...
func DeliveryServiceTenancyTest(t *testing.T) {
	...
	var tenant3DS tc.DeliveryServiceV40
	...
	if _, _, err = tenant4TOClient.UpdateDeliveryServiceV40WithHdr(*tenant3DS.ID, tenant3DS, nil); err == nil {
	...
	if _, _, err = tenant4TOClient.CreateDeliveryServiceV40(tenant3DS); err == nil {
