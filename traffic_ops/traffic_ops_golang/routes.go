...
func Routes(d ServerData) ([]Route, http.Handler, error) {
	...
	return []Route{
		...
		{1.2, http.MethodGet, "servers/{id}/cfg.json$", wrapHeaders(wrapAuth(cfgDiffsHandler(d.DB), d.Insecure, d.TOSecret, rd.PrivLevelStmt, CfgDiffsPrivLevel))},
		//{1.2, http.MethodPost, "servers/{id}/cfg$", wrapHeaders(wrapAuth(postCfgDiffsHandler(d.DB), d.Insecure, d.TOSecret, rd.PrivLevelStmt, CfgDiffsWritePrivLevel))},
	}, rootHandler(d), nil
}
