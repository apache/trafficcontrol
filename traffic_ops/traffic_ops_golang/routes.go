...
func Routes(d ServerData) ([]Route, http.Handler, error) {
	...
	return []Route{
		...
		{1.2, http.MethodGet, "servers/{id}/cfg.json$", wrapHeaders(wrapAuth(getCfgDiffsHandler(d.DB), d.Insecure, d.TOSecret, rd.PrivLevelStmt, CfgDiffsPrivLevel))},
		{1.2, http.MethodPut, "servers/{id}/{cfg}$", wrapHeaders(wrapAuthWithData(putCfgDiffsHandler(d.DB), d.Insecure, d.TOSecret, rd.PrivLevelStmt, CfgDiffsWritePrivLevel))},		
	}, rootHandler(d), nil
}
