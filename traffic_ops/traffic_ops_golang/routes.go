...
func Routes(d ServerData) ([]Route, http.Handler, error) {
	...
	return []Route{
		...
		{1.2, http.MethodGet, "servers/{host-name}/config_diffs.json$", wrapHeaders(wrapAuth(getCfgDiffsHandler(d.DB), d.Insecure, d.TOSecret, rd.PrivLevelStmt, CfgDiffsPrivLevel))},
		{1.2, http.MethodPut, "servers/{host-name}/{cfg-file-name}$", wrapHeaders(wrapAuthWithData(putCfgDiffsHandler(d.DB), d.Insecure, d.TOSecret, rd.PrivLevelStmt, CfgDiffsWritePrivLevel))},		
	}, rootHandler(d), nil
}
