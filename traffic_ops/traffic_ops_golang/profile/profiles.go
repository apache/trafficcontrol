...
func (prof *TOProfile) Read() ([]interface{}, error, error, int) {
	...
	where, orderBy, limit, queryValues, errs := dbhelpers.BuildWhereAndOrderByAndPagination(prof.APIInfo().Params, queryParamsToQueryCols)
	...
	query := selectProfilesQuery() + where + orderBy + limit
