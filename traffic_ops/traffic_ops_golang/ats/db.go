...
func GetServerProfileParamData(tx *sql.Tx, serverName tce.CacheName, configFile string) (map[string]string, error) {
	qry := `
SELECT
  p.name,
  p.value
...
func GetATSMajorVersionFromServerName(tx *sql.Tx, serverName tce.CacheName) (int, error) {
...
// Returns the parameter, whether it existed, and any error.
func GetServerProfileParamValue(tx *sql.Tx, serverName tce.CacheName, configFile string, name string) (string, bool, error) {
	qry := `
SELECT
  p.value
FROM
...
func GetServerNameFromID(tx *sql.Tx, id int) (tce.CacheName, bool, error) {
...
func GetServerNameFromNameOrID(tx *sql.Tx, serverNameOrID string) (tce.CacheName, error, error, int) {
	if serverID, err := strconv.Atoi(serverNameOrID); err == nil {
		...
		return tce.CacheName(serverName), nil, nil, http.StatusOK
	}

	serverName := tce.CacheName(serverNameOrID)
...
func GetServerInfoByHost(tx *sql.Tx, host tce.CacheName) (*atscfg.ServerInfo, bool, error) {
...
func GetServerURISignedDSes(tx *sql.Tx, serverHostName string, serverPort int) ([]tce.DeliveryServiceName, error) {
	qry := `
SELECT
  ds.xml_id
FROM
...
func GetDSNames(tx *sql.Tx, serverHostName string, serverPort int) (map[tce.DeliveryServiceName]struct{}, error) {
	qry := `
SELECT
  ds.xml_id
FROM
...
func GetServerNameAndTypeFromID(tx *sql.Tx, id int) (tce.CacheName, tce.CacheType, bool, error) {
	qry := `
SELECT
  s.host_name,
  tp.name
...
`
	...
	if err := tx.QueryRow(qry, id).Scan(&name, &typ); err != nil {
		if err == sql.ErrNoRows {
			return "", tce.CacheType(""), false, nil
		}
		return "", tce.CacheType(""), false, errors.New("querying: " + err.Error())
	}
	return name, typ, true, nil
}

// GetServerNameAndTypeFromID returns the server's name, type, whether it exists, and any error.
func GetServerTypeFromName(tx *sql.Tx, name tce.CacheName) (tce.CacheType, bool, error) {
	qry := `
SELECT
  tp.name
FROM
...
func GetServerNameAndDomainFromID(tx *sql.Tx, id int) (tce.CacheName, string, bool, error) {
	qry := `
SELECT
  s.host_name,
  s.domain_name
...
// GetServerNameAndDomainFromID returns the server's name, domain, whether it exists, and any error.
func GetServerDomainFromName(tx *sql.Tx, name tce.CacheName) (string, bool, error) {
	qry := `
SELECT
  s.domain_name
FROM
