...
func GetServerHostingOrigins(tx *sql.Tx, serverName tce.CacheName, serverType tce.CacheType) ([]string, error) {
	qry := `
SELECT
...
WHERE
  ...
  AND ds.type IN (SELECT id FROM type WHERE name like '%` + tce.DSTypeLiveNationalSuffix + `')
  AND ds.active = true
...
WHERE
  s.host_name = $1
  AND ds.cdn_id = s.cdn_id
  AND ds.type IN (SELECT id FROM type WHERE (name LIKE '%` + tce.DSTypeLiveSuffix + `' OR name LIKE '%` + tce.DSTypeLiveNationalSuffix + `'))
`
	}
