...
func GetEdgeHeaderRewriteDotConfig(w http.ResponseWriter, r *http.Request) {
	...
	txt := atscfg.MakeHeaderRewriteDotConfig(tce.CDNName(cdnName), toToolName, toURL, ds, assignedEdges)
...
func GetMidHeaderRewriteDotConfig(w http.ResponseWriter, r *http.Request) {
	...
	txt := atscfg.MakeHeaderRewriteMidDotConfig(tce.CDNName(cdnName), toToolName, toURL, ds, assignedMids)
...
`
	...
	for rows.Next() {
		...
		s.Status = tce.CacheStatusFromString(string(s.Status))
