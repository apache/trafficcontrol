...
func GetConfigMetaData(w http.ResponseWriter, r *http.Request) {
	...
	for cfgFile, cfgParams := range locationParams {
		...
		scope, err := getScope(inf.Tx.Tx, cfgFile, server)
		if err != nil {
...
func getScope(tx *sql.Tx, cfgFile string, server ServerInfo) (tc.ATSConfigMetaDataConfigFileScope, error) {
	switch {
	...
	case strings.HasPrefix(cfgFile, "to_ext_") && strings.HasSuffix(cfgFile, ".config"):
		fallthrough
	case cfgFile == "cache.config" && tc.CacheTypeFromString(server.TypeName) == tc.CacheTypeMid:
	...
	case cfgFile == "astats.config":
		fallthrough
	case cfgFile == "cache.config": // MUST come after the above 'cache.config && mid' check
		fallthrough
...
`
	s := ServerInfo{ID: id}
	if err := tx.QueryRow(qry, id).Scan(&s.CDN, &s.CDNID, &s.HostName, &s.IP, &s.ProfileID, &s.ProfileName, &s.Port, &s.TypeName); err != nil {
		if err == sql.ErrNoRows {
