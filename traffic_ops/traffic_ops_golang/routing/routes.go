...
func Routes(d ServerData) ([]Route, []RawRoute, http.Handler, error) {
	...
	routes := []Route{
		...
		{api.Version{4, 0}, http.MethodGet, `acme_accounts/providers?$`, acme.ReadProviders, auth.PrivLevelOperations, Authenticated, nil, 4034390565},
		{api.Version{4, 0}, http.MethodPost, `deliveryservices/sslkeys/generate/acme/?$`, deliveryservice.GenerateAcmeCertificates, auth.PrivLevelOperations, Authenticated, nil, 2534390576},
