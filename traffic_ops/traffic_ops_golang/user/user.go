...
func (this *TOUser) Read() ([]interface{}, error, error, int) {
	...
	inf := this.APIInfo()
	where, orderBy, limit, queryValues, errs := dbhelpers.BuildWhereAndOrderByAndPagination(inf.Params, this.ParamColumns())
	if len(errs) > 0 {
	...
	query := this.SelectQuery() + where + orderBy + limit
