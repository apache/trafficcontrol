...
`
	...
	rows, err := tx.Query(selectQuery, tc.CacheStatusOffline, hostName)
	if err != nil {
...
`
	...
	if hostName == "all" {
		...
		rows, err = tx.Query(baseSelectStatement+` WHERE s.host_name = $2`+groupBy, tc.CacheStatusOffline, hostName)
		if err != nil {
