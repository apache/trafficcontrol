...
func (ssc *TOServerServerCapability) Delete() (error, error, int) {
	...
	if ssc.ServerID != nil {
		cdnName, err := dbhelpers.GetCDNNameFromServerID(ssc.Info().Tx.Tx, int64(*ssc.ServerID))
		if err != nil {
			return nil, err, http.StatusInternalServerError
		}
		userErr, sysErr, errCode := dbhelpers.CheckIfCurrentUserCanModifyCDN(ssc.Info().Tx.Tx, string(cdnName), ssc.Info().User.UserName)
		if userErr != nil || sysErr != nil {
...
func (ssc *TOServerServerCapability) Create() (error, error, int) {
	...
	userErr, sysErr, errCode := dbhelpers.CheckIfCurrentUserCanModifyCDN(tx.Tx, string(cdnName), ssc.Info().User.UserName)
	if userErr != nil || sysErr != nil {
		return userErr, sysErr, errCode
	}
