...
func addResolvers(allFederations []tc.IAllFederation, feds []FedInfo, fedsResolvers map[int][]FedResolverInfo) []tc.IAllFederation {
	dsFeds := map[tce.DeliveryServiceName][]tc.FederationResolverMapping{}
	for _, fed := range feds {
		...
		for _, resolver := range fedsResolvers[fed.ID] {
			...
			case tce.FederationResolverType4:
				mapping.Resolve4 = append(mapping.Resolve4, resolver.IP)
			case tce.FederationResolverType6:
...
type FedResolverInfo struct {
	Type tce.FederationResolverType
	IP   string
}
...
`
	...
	for rows.Next() {
		...
		f.Type = tce.FederationResolverTypeFromString(fType)
...
func addFederationResolverMappingsToFederation(res tc.ResolverMapping, xmlid string, fed uint, tx *sql.Tx) (string, error) {
	...
	if len(res.Resolve4) > 0 {
		inserted, err := addFederationResolver(res.Resolve4, tce.FederationResolverType4, fed, tx)
		if err != nil {
			return "", err
		}
	...
	if len(res.Resolve6) > 0 {
		inserted, err := addFederationResolver(res.Resolve6, tce.FederationResolverType6, fed, tx)
		if err != nil {
			return "", err
		}
		resp += strings.Join(inserted, ", ")
...
func addFederationResolver(res []string, t tce.FederationResolverType, fedID uint, tx *sql.Tx) ([]string, error) {
	inserted := []string{}
	for _, r := range res {
		var ip string
		var id uint
