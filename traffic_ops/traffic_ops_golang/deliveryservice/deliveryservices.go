...
func createV14(w http.ResponseWriter, r *http.Request, inf *api.APIInfo, reqDS tc.DeliveryServiceNullableV14) (*tc.DeliveryServiceNullableV14, int, error, error) {
	...
	if c, err := createConsistentHashQueryParams(tx, *ds.ID, ds.ConsistentHashQueryParams); err != nil {
		...
		api.CreateChangeLogRawTx(api.ApiChange, "DS: "+*ds.XMLID+", ID: "+string(*ds.ID)+", ACTION: Created "+string(c)+" consistent hash query params", user, tx)
	}
	...
	if err := api.CreateChangeLogRawErr(api.ApiChange, "DS: "+*ds.XMLID+", ID: "+string(*ds.ID)+", ACTION: Created delivery service", user, tx); err != nil {
...
func updateV14(w http.ResponseWriter, r *http.Request, inf *api.APIInfo, reqDS *tc.DeliveryServiceNullableV14) (*tc.DeliveryServiceNullableV14, int, error, error) {
	...
	if res, err := tx.Exec(q, *ds.ID); err != nil {
		...
		api.CreateChangeLogRawTx(api.ApiChange, "DS: "+*ds.XMLID+", ID: "+string(*ds.ID)+", ACTION: Deleted "+string(c)+" consistent hash query params", user, tx)
	}
...
func updatePrimaryOrigin(tx *sql.Tx, user *auth.CurrentUser, ds tc.DeliveryServiceNullable) error {
	...
	if ds.OrgServerFQDN == nil || *ds.OrgServerFQDN == "" {
		if count == 1 {
			...
			api.CreateChangeLogRawTx(api.ApiChange, "DS: "+*ds.XMLID+", ID: "+string(*ds.ID)+", ACTION: Deleted primary origin", user, tx)
		}
		return nil
	}
	...
	api.CreateChangeLogRawTx(api.ApiChange, "DS: "+*ds.XMLID+", ID: "+string(*ds.ID)+", ACTION: Updated primary origin: "+name, user, tx)
...
func createPrimaryOrigin(tx *sql.Tx, user *auth.CurrentUser, ds tc.DeliveryServiceNullable) error {
	...
	api.CreateChangeLogRawTx(api.ApiChange, "DS: "+*ds.XMLID+", ID: "+string(*ds.ID)+", ACTION: Created primary origin id: "+strconv.Itoa(originID), user, tx)
