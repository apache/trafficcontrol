...
func AddSSLKeys(w http.ResponseWriter, r *http.Request) {
	...
	if !inf.Config.RiakEnabled {
		api.HandleErr(w, r, inf.Tx, http.StatusInternalServerError, nil, errors.New("adding Riak SSL keys for delivery service:: riak is not configured"))
		return
	}
	...
	if userErr, sysErr, errCode := tenant.Check(inf.User, keysObj.DeliveryService, inf.Tx); userErr != nil || sysErr != nil {
		api.HandleErr(w, r, inf.Tx, errCode, userErr, sysErr)
		return
	}
	certChain, err := verifyAndEncodeCertificate(keysObj.Certificate.Crt, "")
	if err != nil {
		api.HandleErr(w, r, inf.Tx, http.StatusBadRequest, errors.New("verifying certificate: "+err.Error()), nil)
		return
	}
	keysObj.Certificate.Crt = certChain
	if err := riaksvc.PutDeliveryServiceSSLKeysObj(keysObj, inf.Tx, inf.Config.RiakAuthOptions); err != nil {
		api.HandleErr(w, r, inf.Tx, http.StatusBadRequest, nil, errors.New("putting Riak SSL keys for delivery service '"+keysObj.DeliveryService+"': "+err.Error()))
		return
	}
...
func GetSSLKeysByHostName(w http.ResponseWriter, r *http.Request) {
	...
	// lookup the cdnID
	cdnID, ok, err := getCDNIDByDomainname(domainName, inf.Tx)
	if err != nil {
		api.HandleErr(w, r, inf.Tx, http.StatusInternalServerError, nil, errors.New("getting cdn id by domain name: "+err.Error()))
		return
	}
	if !ok {
	...
	xmlID, ok, err := getXMLID(cdnID, hostRegex, inf.Tx)
	if err != nil {
		api.HandleErr(w, r, inf.Tx, http.StatusInternalServerError, nil, errors.New("getting xml id: "+err.Error()))
		return
	}
	if !ok {
	...
	keyObj, ok, err := riaksvc.GetDeliveryServiceSSLKeysObj(xmlID, version, inf.Tx, inf.Config.RiakAuthOptions)
	if err != nil {
		api.HandleErr(w, r, inf.Tx, http.StatusInternalServerError, nil, errors.New("getting ssl keys: "+err.Error()))
		return
	}
	if !ok {
...
func GetSSLKeysByXMLID(w http.ResponseWriter, r *http.Request) {
	...
	defer inf.Close()
	if inf.Config.RiakEnabled == false {
		api.HandleErr(w, r, inf.Tx, http.StatusServiceUnavailable, errors.New("The RIAK service is unavailable"), errors.New("getting Riak SSL keys by xml id: riak is not configured"))
		return
	}
	version := inf.Params["version"]
...
func DeleteSSLKeys(w http.ResponseWriter, r *http.Request) {
	...
	if err := riaksvc.DeleteDSSSLKeys(inf.Tx, inf.Config.RiakAuthOptions, ds, inf.Params["version"]); err != nil {
		api.HandleErr(w, r, inf.Tx, http.StatusInternalServerError, userErr, errors.New("deliveryservice.DeleteSSLKeys: deleting SSL keys: "+err.Error()))
		return
	}
