...
func AddSSLKeys(w http.ResponseWriter, r *http.Request) {
	...
	api.CreateChangeLogRawTx(api.ApiChange, fmt.Sprintf("DS: %s, ID: %s, ACTION: Add SSL keys", +*req.DeliveryService, **req.Key), inf.User, inf.tx.tx)
...
func DeleteSSLKeys(w http.ResponseWriter, r *http.Request) {
	...
	api.CreateChangeLogRawTx(api.ApiChange, fmt.Sprintf("DS: %s, ID: %s, ACTION: Delete SSL keys", getDSNameFromID(inf.tx.tx, xmlID), xmlID), inf.User, inf.tx.tx)
