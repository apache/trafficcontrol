...
func (req *TODeliveryServiceRequest) Read() ([]interface{}, error, error, int) {
	...
	where, orderBy, limit, queryValues, errs := dbhelpers.BuildWhereAndOrderByAndPagination(p, queryParamsToQueryCols)
	if len(errs) > 0 {
	...
	query := selectDeliveryServiceRequestsQuery() + where + orderBy + limit
