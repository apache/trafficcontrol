...
// GetID is part of the common.Identifier interface
...
// GetAuditName is part of the common.Identifier interface
func (req TODeliveryServiceRequest) GetAuditName() string {
	return req.getXMLID()
}

// GetType is part of the common.Identifier interface
func (req TODeliveryServiceRequest) GetType() string {
	return "deliveryservice_request"
}

// SetID is part of the common.Identifier interface
func (req *TODeliveryServiceRequest) SetID(i int) {
	req.ID = &i
}

// Read implements the api.Reader interface
func (req *TODeliveryServiceRequest) Read(db *sqlx.DB, parameters map[string]string, user auth.CurrentUser) ([]interface{}, []error, common.ApiErrorType) {
	...
	return deliveryServiceRequests, []error{}, common.NoError
}
...
// Update implements the common.Updater interface.
...
func (req *TODeliveryServiceRequest) Update(db *sqlx.DB, user auth.CurrentUser) (error, common.ApiErrorType) {
	var current TODeliveryServiceRequest
	if req.ID == nil {
		log.Errorf("error updating DeliveryServiceRequest: ID is nil")
		return errors.New("error updating DeliveryServiceRequest: ID is nil"), common.DataMissingError
	}
	err := db.QueryRowx(selectDeliveryServiceRequestsQuery() + `WHERE r.id=` + strconv.Itoa(*req.ID)).StructScan(&current)
	if err != nil {
		log.Errorf("Error querying DeliveryServiceRequests: %v", err)
		return err, common.SystemError
	}
	...
	if current.Status == nil || req.Status == nil {
		return errors.New("Missing status for DeliveryServiceRequest"), common.DataMissingError
	}
	...
	if rowsAffected < 1 {
		return errors.New("no deliveryservice request found with this id"), common.DataMissingError
...
// Creator implements the common.Creator interface
...
func (req *TODeliveryServiceRequest) Create(db *sqlx.DB, user auth.CurrentUser) (error, common.ApiErrorType) {
	if req == nil {
		return errors.New("nil deliveryservice_request"), common.SystemError
	}
	if req.Status == nil {
		return errors.New("nil deliveryservice_request status"), common.SystemError
	}
	if *req.Status != tc.RequestStatusDraft && *req.Status != tc.RequestStatusSubmitted {
	...
	if ds == nil {
		log.Debugln(" -- no ds")
		return errors.New("no delivery service associated with this request"), common.DataMissingError
	}
	if ds.XMLID == nil {
		log.Debugln(" -- no XMLID")
		return errors.New("no xmlId associated with this request"), common.DataMissingError
	}
	XMLID := *ds.XMLID
	...
	if active {
		return errors.New(`An active request exists for delivery service '` + XMLID + `'`), common.DataConflictError
	}
...
func (req *TODeliveryServiceRequest) Delete(db *sqlx.DB, user auth.CurrentUser) (error, common.ApiErrorType) {
	var st tc.RequestStatus
	log.Debugln("DELETING REQUEST WITH ID ", strconv.Itoa(*req.ID))
	if req.ID == nil {
		return errors.New("cannot delete deliveryservice_request -- ID is nil"), common.SystemError
	}
	...
	if st == tc.RequestStatusComplete || st == tc.RequestStatusPending || st == tc.RequestStatusRejected {
		return fmt.Errorf("cannot delete a deliveryservice_request with state %s", string(st)), common.DataConflictError
	}
	...
	if rowsAffected < 1 {
		...
		return errors.New("no request with that id found"), common.DataMissingError
	}
	if rowsAffected > 1 {
		log.Errorln("the delete affected too many rows")
		return fmt.Errorf("this delete affected too many rows: %d", rowsAffected), common.SystemError
	}
	err = tx.Commit()
	if err != nil {
		log.Errorln("Could not commit transaction: ", err)
...
func (req *deliveryServiceRequestAssignment) Update(db *sqlx.DB, user auth.CurrentUser) (error, common.ApiErrorType) {
	...
	if req.ID == nil {
		return errors.New("cannot update DeliveryServiceRequestAssignment -- ID is nil"), common.SystemError
	}
...
// Update status only
func (req *deliveryServiceRequestStatus) Update(db *sqlx.DB, user auth.CurrentUser) (error, common.ApiErrorType) {
	// req represents the state the deliveryservice_request is to transition to
	...
	if err = current.Status.ValidTransition(*req.Status); err != nil {
		return err, common.DataConflictError
	}
