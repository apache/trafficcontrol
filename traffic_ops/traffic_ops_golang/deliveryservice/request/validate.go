...
func (req *TODeliveryServiceRequest) Validate() error {
	...
	if req.ID != nil && *req.ID > 0 {
		err := req.APIInfo().Tx.QueryRow(`SELECT status FROM deliveryservice_request WHERE id=` + strconv.Itoa(*req.ID)).Scan(&fromStatus)
	...
	e := req.DeliveryService.Validate(req.APIInfo().Tx)
