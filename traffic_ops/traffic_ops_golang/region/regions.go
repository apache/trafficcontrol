...
func (region *TORegion) Read(db *sqlx.DB, parameters map[string]string, user auth.CurrentUser) ([]interface{}, []error, common.ApiErrorType) {
	var rows *sqlx.Rows
	...
	return regions, []error{}, common.NoError
}
...
func (region *TORegion) Update(db *sqlx.DB, user auth.CurrentUser) (error, common.ApiErrorType) {
	...
	if rowsAffected != 1 {
		if rowsAffected < 1 {
			return errors.New("no region found with this id"), common.DataMissingError
		}
...
func (region *TORegion) Create(db *sqlx.DB, user auth.CurrentUser) (error, common.ApiErrorType) {
	rollbackTransaction := true
	tx, err := db.Beginx()
	defer func() {
		if tx == nil || !rollbackTransaction {
...
func (region *TORegion) Delete(db *sqlx.DB, user auth.CurrentUser) (error, common.ApiErrorType) {
	rollbackTransaction := true
	tx, err := db.Beginx()
	defer func() {
		if tx == nil || !rollbackTransaction {
	...
	if rowsAffected < 1 {
		return errors.New("no region with that id found"), common.DataMissingError
	}
	if rowsAffected > 1 {
