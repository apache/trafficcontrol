...
AND d.active = true
`
	q += fmt.Sprintf(" and t.name != '%s'", tce.DSTypeAnyMap)
	...
	for rows.Next() {
		...
		if deepCachingType.Valid {
			...
			t := tce.DeepCachingTypeFromString(deepCachingType.String)
		...
		ds.StaticDNSEntries = staticDNSEntries[tce.DeliveryServiceName(xmlID)]
...
func getStaticDNSEntries(cdn string, tx *sql.Tx) (map[tce.DeliveryServiceName][]tc.CRConfigStaticDNSEntry, error) {
	entries := map[tce.DeliveryServiceName][]tc.CRConfigStaticDNSEntry{}
...
`
	...
	for rows.Next() {
		...
		entries[tce.DeliveryServiceName(ds)] = append(entries[tce.DeliveryServiceName(ds)], tc.CRConfigStaticDNSEntry{Name: name, TTL: ttl, Value: value, Type: ttype})
	}
	return entries, nil
}
