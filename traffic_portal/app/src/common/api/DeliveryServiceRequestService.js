...
var DeliveryServiceRequestService = function($http, locationUtils, messageModel, ENV) {
	...
	this.assignDeliveryServiceRequest = function(id, userId) {
		return $http.put(ENV.api.stable + "deliveryservice_requests/" + id + "/assign", { id: id, assigneeId: userId }).then(
			function(result) {
				return result;
			},
			function(err) {
	...
	this.updateDeliveryServiceRequestStatus = function(id, status) {
		return $http.put(ENV.api.stable + "deliveryservice_requests/" + id + "/status", { id: id, status: status }).then(
			function(result) {
				return result;
			},
			function(err) {
