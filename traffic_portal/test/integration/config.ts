...
export let randomize = Math.random().toString(36).substring(3, 7);
...
config.beforeLaunch = async function () {
	// This is the only way to carry over this value into the spec runners -
	// otherwise for some reason `randomize` gets re-generated by the time the
	// config.onPrepare is run.
	process.env.randomize = randomize;
	await api.Login();
	await setupAPI();
};

config.onPrepare = async function () {
	const rand = process.env.randomize;
	if (!rand) {
		throw new Error("environment dropped 'randomize' between launch and prep");
	}
	randomize = rand;
	testingConfig.randomize = rand;
