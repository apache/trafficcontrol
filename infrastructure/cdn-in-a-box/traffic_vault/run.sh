...
if [[ -f $1 ]]; then
    ...
    echo "Missing function definition file"
    exit 1
fi
...
init() {
    # Set the Riak certs in the config; generated in run.sh
    sed -i \
        -e 's%^## ssl.\(certfile\|keyfile\|cacertfile\) =%ssl.\1 =%' \
        -e 's%^## \(listener.https.internal\)%\1%g' \
        -e 's%$(platform_etc_dir)/%/etc/riak/certs/%'  \
        -e "s%127.0.0.1%$(hostname -f)%g" \
        /etc/riak/riak.conf && \
        echo 'tls_protocols.tlsv1 = on' >> /etc/riak/riak.conf 

    while ! nc $TO_HOST $TO_PORT </dev/null 2>/dev/null; do
        echo "Waiting for $TO_HOST"
        sleep 3
    done

    # test communication with traffic_ops
    to-get api/1.2/cdns | jq -S .response
#	TMP_IP=$IP
#	TMP_DOMAIN=$DOMAIN
#	TMP_GATEWAY=$GATEWAY
#
