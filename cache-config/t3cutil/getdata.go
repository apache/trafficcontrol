...
func GetPackages(cfg TCCfg) ([]Package, error) {
	...
	params, _, err := cfg.TOClient.GetServerProfileParameters((*server.Profiles)[0], nil)
	if err != nil {
		return nil, errors.New("getting server profile '" + (*server.Profiles)[0] + "' parameters: " + err.Error())
	}
	packages := []Package{}
