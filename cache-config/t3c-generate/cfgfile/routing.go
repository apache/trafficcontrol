...
func GetConfigFile(toData *t3cutil.ConfigData, fileInfo atscfg.CfgMeta, hdrCommentTxt string, thiscfg config.Cfg) (string, string, bool, string, atscfg.CfgMetaData, []string, error) {
	start := time.Now()
	defer func() {
	...
	return cfg.Text, cfg.ContentType, cfg.Secure, cfg.LineComment, cfg.MetaData, cfg.Warnings, nil
}
