...
function run(): number {
	...
	parser.add_argument("-k", "--insecure", {
		action: "storeTrue",
		help: "Skip Traffic Ops server certificate validation. This affects requests from Traffic Portal to Traffic Ops AND signature verification of any passed SSL keys/certificates"
	});
	...
	parser.add_argument("-c", "--cert-path", {
		dest: "certPath",
		help: "Specify a location for an SSL certificate to be used by Traffic Portal. (Requires `-K`/`--key-path`. If both are omitted, will serve using HTTP)",
		type: "str"
	});
	parser.add_argument("-K", "--key-path", {
		dest: "keyPath",
		help: "Specify a location for an SSL certificate to be used by Traffic Portal. (Requires `-c`/`--cert-path`. If both are omitted, will serve using HTTP)",
		type: "str"
	});
	...
	if (config.useSSL) {
		...
		createServer(
			...
			server).listen(config.port, ()=> {console.log(`Node Express server listening on port ${config.port}`); });
		try {
