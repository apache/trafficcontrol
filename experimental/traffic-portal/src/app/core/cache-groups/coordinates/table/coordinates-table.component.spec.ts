...
describe("CoordinatesTableComponent", () => {
	...
	it("handles unrecognized contextmenu events", () => {
		expect(async () => component.handleContextMenu({
			...
			data: {id: 1, lastUpdated: new Date(), latitude: 0, longitude: 0, name: "Div"}
		})).not.toThrow();
	});
	...
	it("handles the 'delete' context menu item", fakeAsync(async () => {
		...
		const crd = await api.createCoordinate({latitude: 0, longitude: 0, name: "test"});
		expect(openSpy).not.toHaveBeenCalled();
		const asyncExpectation = expectAsync(component.handleContextMenu({action: "delete", data: crd})).toBeResolvedTo(undefined);
		tick();
