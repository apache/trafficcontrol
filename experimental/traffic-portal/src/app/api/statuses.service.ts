...
export class StatusesService extends APIService {
	 ...
	 */
	public async getStatuses(id?: number | string): Promise<Array<ResponseStatus> | ResponseStatus> {
		const path = "statuses";
		if (id !== undefined) {
			let statuses;
			statuses = await this.get<[ResponseStatus]>(path, undefined, { id: String(id) }).toPromise();
	 ...
	 */
	public async updateStatus(data: ResponseStatus, id: number): Promise<ResponseStatus | undefined> {
