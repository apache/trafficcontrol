...
func createCacheStats(client influx.Client, replication *string) {
	...
	createContinuousQuery(client, "bandwidth_cdn_type_1min", "CREATE CONTINUOUS QUERY bandwidth_cdn_type_1min ON cache_stats RESAMPLE FOR 5m BEGIN SELECT sum(value) AS \"value\" INTO \"cache_stats\".\"monthly\".\"bandwidth.cdn.1min\" FROM \"cache_stats\".\"monthly\".\"bandwidth.1min\" GROUP BY time(1m), cdn, type END")
	createContinuousQuery(client, "connections_cdn_type_1min", "CREATE CONTINUOUS QUERY connections_cdn_type_1min ON cache_stats RESAMPLE FOR 5m BEGIN SELECT sum(value) AS \"value\" INTO \"cache_stats\".\"monthly\".\"connections.cdn.1min\" FROM \"cache_stats\".\"monthly\".\"connections.1min\" GROUP BY time(1m), cdn, type END")
}
