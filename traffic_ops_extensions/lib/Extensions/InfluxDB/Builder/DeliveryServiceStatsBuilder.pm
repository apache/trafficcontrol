...
use File::Slurp;
use Math::Round;
use Extensions::InfluxDB::Builder::InfluxDBBuilder;
...
sub validate_keys {
	...
	return Extensions::InfluxDB::Builder::InfluxDBBuilder->validate_keys( $args, $valid_keys );
}
...
sub summary_query {
	...
	if ( $self->validate_keys() ) {
		...
		my $end_date = Extensions::InfluxDB::Builder::InfluxDBBuilder->to_influxdb_date( $args->{endDate} );
		...
		$query = Extensions::InfluxDB::Builder::InfluxDBBuilder->append_clauses( $query, $args );
		return Extensions::InfluxDB::Builder::InfluxDBBuilder->clean_whitespace($query);
	}
}
