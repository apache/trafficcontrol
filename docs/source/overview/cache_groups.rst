...
- "Deep Coverage Zone File" (alias ``DEEP_CZ`` in source code, database entries, and :ref:`to-api` requests/responses) allows Traffic Router to direct clients to this Cache Group if they were assigned a geographic location by looking up their IP addresses in the :term:`Deep Coverage Zone File`.

	.. warning:: In order to make use of "deep caching" for a :term:`Delivery Service`, this localization method must be enabled on the Cache Groups that contain servers assigned to said :term:`Delivery Service` **and** that :term:`Delivery Service` must have :ref:`ds-deep-caching` enabled.
