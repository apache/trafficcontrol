...
:hypnotoad: This is a group of options used nearly exclusively by the `Legacy Perl Script`_ (as the name implies).
	...
	:workers:           This is used only by the `Legacy Perl Script`_, and has no known purpose. Though, one could guess that it's the number of concurrent HTTP server workers allowed.
