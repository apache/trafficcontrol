...
Building
========
Traffic Router uses the `Maven <http://maven.apache.org/guides/getting-started/>` build system. The project also provides the ``traffic_router_rpm`` project which can be used to create an RPM package directly. In most cases, though, it is recommended that Traffic Router be built using the instructions in :ref:`dev-building`.
